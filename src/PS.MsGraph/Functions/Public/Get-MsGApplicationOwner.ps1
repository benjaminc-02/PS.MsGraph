function Get-MsGApplicationOwner {
    <#
    .SYNOPSIS
    Retrieves owners of an app registrations.
    .DESCRIPTION
    This function retrieves the owner principals of a specified app registration.
    ...
    PROCESS {
        ...
        # Retrieve Owners for Application
        try {
            $graphEndpoint = 'applications/{0}/owners' -f $application.id
            ...
            $errorMessage = Get-MsGErrorMessage $_
            $errorException = '{0} : Unable to retrieve application owners from Entra ID. Error: {1}' -f $MyInvocation.MyCommand.Name, $errorMessage
            throw $errorException
        }
        # end region
    }
