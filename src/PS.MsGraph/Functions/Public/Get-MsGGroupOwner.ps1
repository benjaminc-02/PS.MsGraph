function Get-MsGGroupOwner {
    <#
    .SYNOPSIS
    Retrieves owners of a group.
    .DESCRIPTION
    This function retrieves the owner principals of a specified group.
    ...
    PROCESS {
        ...
        # Retrieve group owners for group.
        try {
            $graphEndpoint = 'groups/{0}/owners' -f $group.id
            ...
            $errorMessage = Get-MsGErrorMessage $_
            $errorException = '{0} : Unable to retrieve group owners from Entra ID. Error: {1}' -f $MyInvocation.MyCommand.Name, $errorMessage
            throw $errorException
        }
        # end region
    }
